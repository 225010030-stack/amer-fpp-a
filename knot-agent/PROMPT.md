...
| 场景 | 执行示例 |
|------|----------|
| US Anthem Medical（默认） | `痛点1 上传输入/人数.csv 202607 US 145411.03` |
| US Cigna Dental | `痛点1 上传输入/人数.csv 202607 US 50000 Cigna Dental` |
| CAN SunLife（默认） | `痛点1 上传输入/人数.csv 202607 CAN 50000` |

回复须含：`【供应商】Cigna · Dental`（或实际 vendor/fee_type）  
提单/台账(4/6)：用 **该国预填表**（US→`测试文档/模板/US/` 或 `闭环CSV/US/`；CAN→`…/CAN/`），CSV 内字段应与 vendor 一致。
...
| 数字 | 指令 | 要点 |
|------|------|------|
| 0 | CAN演示 | 培训（样例 `闭环CSV/CAN/CAN_prefill_*.csv`） |
| 1 | 目录预检 \<月\> **CAN** | 仅 CAN/ |
...
闭环顺序：**4→2→3→5→6**（各国独立跑，文件建议文件名带 `_US_` / `_CAN_`）  
上传：`上传输入/` → 给路径
...
- 正式 US：`US 2 上传输入/人数_US.csv 202607 US 145411.03 Cigna Dental`
- 正式 CAN：`CAN 2 上传输入/人数_CAN.csv 202607 CAN 50000`
