...
| 0 | US演示 | 培训 |
| 1 | 目录预检 \<月\> **US** | 仅 USA/ |
| 2 | 痛点1 … **US** … [Vendor] [FeeType] | **单家**分摊；**正式月建议网页步骤5「批量分摊」**（台账 P4 + P1 一次跑全部供应商） |
...
| 1 | 目录预检 \<月\> **CAN** | 仅 CAN/ |
| 2 | 痛点1 … **CAN** … [Vendor] [FeeType] | 默认 SunLife；**正式月建议网页步骤5批量分摊** |
...
- 多 CSV / 2.0 全流程：**网页入口US** / **网页入口CAN**（upload-docs；步骤5 **批量分摊** 为主，步骤4 账单可选 local/knot_agent）
